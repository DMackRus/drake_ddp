...
method_names = ["Baseline", "SI5", "SI10", "SI20", "adaptiveJerk5", "iterativeError5"]
methods = [utils_derivs_interpolation.derivs_interpolation("setInterval", 1, 0, 0, 0),
            ...
            utils_derivs_interpolation.derivs_interpolation("setInterval", 10, 0, 0, 0),
            utils_derivs_interpolation.derivs_interpolation("setInterval", 20, 0, 0, 0),
            utils_derivs_interpolation.derivs_interpolation("adaptiveJerk", 5, 40, 1e-1, 0),
            utils_derivs_interpolation.derivs_interpolation("iterativeError", 5, 0, 0, 1000)]
...
for i in range(len(method_names)):
    ...
    ilqr = IterativeLinearQuadraticRegulator(system_, num_steps, 
        beta=0.5, delta=1e-2, gamma=0, derivs_keypoint_method=methods[i])
    ...
    if opt_failed:
        ...
        opt_times.append(solve_time)
        cost = 0
        for i in range(len(x)):
            cost += (x[:,i]-x_nom).T@Q@(x[:,i]-x_nom) + u[:,i].T@R@u[:,i]
        final_costs.append(cost)

plot_cheetah("mini_cheetah", method_names, opt_times, final_costs)
