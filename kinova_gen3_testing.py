...
# def plot(task_name, method_names, opt_times, cost_reductions, num_iterations, avg_percent_derivs):
...
#     axs[3].set_ylabel('average percent of derivatives')
...
if optimize:
    ...
    for i in range(len(method_names)):
        ilqr = IterativeLinearQuadraticRegulator(system_, num_steps, 
            beta=0.5, delta=1e-3, gamma=0, derivs_keypoint_methods = interpolation_method)
    ...
    # Solve the optimization problem
        new_interpolation_method = methods[i]
        ilqr.SetDerivativeInterpolator(new_interpolation_method)
        ...
        avg_percent_derivs.append(avg_percent_deriv)
